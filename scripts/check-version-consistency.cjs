...
function main() {
  ...
  if (packageVersion !== tauriVersion) {
    log(`❌ Version mismatch between package.json (${packageVersion}) and tauri.conf.json (${tauriVersion})`, 'red');
    hasErrors = true;
  } else {
    log(`✅ package.json and tauri.conf.json versions match: ${packageVersion}`, 'green');
  }

  // Check package.json vs Cargo.toml
  if (cargoVersion && packageVersion !== cargoVersion) {
    log(`❌ Version mismatch between package.json (${packageVersion}) and Cargo.toml (${cargoVersion})`, 'red');
    hasErrors = true;
  } else if (cargoVersion) {
    log(`✅ package.json and Cargo.toml versions match: ${packageVersion}`, 'green');
  }

  // Check changelog version if found
  if (changelogVersion) {
    if (packageVersion !== changelogVersion) {
      log(`❌ Version mismatch between package.json (${packageVersion}) and CHANGELOG.md (${changelogVersion})`, 'red');
      hasErrors = true;
    } else {
      log(`✅ package.json and CHANGELOG.md versions match: ${packageVersion}`, 'green');
    }
  } else {
...
module.exports = { readJsonFile, readChangelogVersion, readCargoToml };
