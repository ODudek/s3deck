use crate::{
    ...
    models::{
        BucketConfig, DeleteResponse, FileItem, ObjectMetadata, Result, S3DeckError,
        UploadFileInfo, UploadRequest, UploadResponse,
    },
    s3_client::S3Client,
};
use std::path::Path;
...
pub async fn upload_files(request: UploadRequest) -> Result<UploadResponse> {
    ...
    for file_path in &request.files {
        ...
        if path.is_dir() {
            ...
            match upload_directory(&s3_client, &request.base_path, &request.current_path, file_path).await {
                Ok(mut dir_results) => {
    ...
    let message = if failed_count == 0 {
        ...
        format!("Uploaded {} file(s), {} failed", success_count, failed_count)
    };
...
fn upload_directory<'a>(
    ...
    if let Ok(entries) = std::fs::read_dir(dir) {
        ...
        return Err(S3DeckError::Io(format!("Cannot read directory: {}", dir_path)));
    }
